...
### 4.7 中文字体决定：HZK12 + Monaco 混排（候选，未真机验证）
...
这轮比较全部基于预生成图片的肉眼判断，**HZK12 这份字形数据本身没有上过真机**。之所以认为预览可信，是因为渲染流程本身（`fontmode="1"`、整数坐标、单色栅格化、无 dithering）已经在 4.5/4.6 节的黑白文字上真机验证过，变的只是字形数据来源。
...
文件取自 GitHub `zfj-hash/BitmapFont` 仓库的 `font/HZK12`（196,272 字节 = 87 区 × 94 位 × 24 字节，无文件头；已用"区16位1=啊"核对偏移量正确）。版权归属国标点阵体系，方正代管授权，网上流传的拷贝授权状态不清楚——个人项目不打算分发，暂不处理，公开发布前要单独核实。
...
- 14px `Hiragino Sans GB W3` 单色栅格化已真机验证：18px 行高可排 7 行；后续方向已改为 HZK12 + Monaco 混排（见 4.7），未真机验证
- 10px `Monaco` 单色栅格化已真机验证：14px 行高适合作为英文和数字的默认正文配置

**待办（按优先级）**
1. 将当前源码提交到个人 GitHub 仓库。
2. 真机验证 HZK12（12×12 国标点阵）+ Monaco 混排方案（见 4.7）：确认真实面板笔画不糊、基线对齐效果与预览一致。

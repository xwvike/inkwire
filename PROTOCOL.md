...
- `examples/gallery/` 回答「上层随手丢一张图进来怎么办」。`ProfileImage` 测量三个量——中间调占比、Otsu 阈值、彩色像素占比——`SuggestOptions` 据此给出建议；`DrawImage` **不会自己去测**，判错藏在绘制里没法排查。测量与决定分开，是为了让判错可见。
  ...
  - **彩色像素占比** 决定要不要关红色。红平面判定 `R>170 且 G<170` 会在木质、米色这类暖色上误触发，在本无红色的照片上撒红点。实测近乎单色的照片只有 3%，而任何真有颜色的素材都超过 12%，5% 一刀即可。对应 `ImageOptions.DisableRed`。
  - **`EnhanceContrast`** 是深色照片的前处理，见 4.10。图形不需要，用了反而更糟。

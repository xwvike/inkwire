...
- `examples/gallery/` 回答「上层随手丢一张图进来怎么办」。`ProfileImage` 测量三个量——中间调占比、Otsu 阈值、彩色像素占比——`SuggestOptions` 据此给出建议；`DrawImage` **不会自己去测**，判错藏在绘制里没法排查。测量与决定分开，是为了让判错可见。
  ...
  - **色相承载结构** 决定用哪个通道当墨量。朋友圈图标是 8 片同亮度不同色相的粉彩花瓣，黄色亮度 218、青色 197，都在 Otsu 的 185 之上被判成白 → **整片消失**。判据是「离纸距离大于 60 却仍落在阈值亮侧」的像素占比：非照片素材里朋友圈 31.7%，其余全部低于 6%。命中时改用 `ToneByColourDistance`，把「离白色多远」当作墨量而不是亮度。
    **色调重写会让原图算出的阈值失效**，必须用 `SuggestOptionsFor` 从重写后的图重新取切点。这个坑我先踩过一次：`ToneByColourDistance` 最初把红色像素替换成纯红 `(255,0,0)`，而纯红亮度是 54，直接把重算的直方图带偏。现在改为透传原色——红色判定照样命中，亮度保持真实。

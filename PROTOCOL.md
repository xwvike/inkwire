...
- `Pattern` 是重复平铺的墨色 tile，用符文网格声明（未映射的符文表示留空，因此 hatch 可以叠在已有内容上）。它是无灰阶下区分区域的确定性手段：不依赖周边像素，可以紧挨文字而不干扰它。**在这块屏的像素间距下，图案读出来是可见纹理而不是灰阶**，用途是区分区域而非模拟中间调。
- `FillPattern` 只填矩形；要做成形状就先 `ClipPath` 再填。tile 锚定在 Frame 坐标而非矩形坐标，所以相邻两块填同一图案时接缝处连续，平移画布也不会拖动纹理相位——与裁剪的坐标契约一致。
...
- `examples/gallery/` 回答「上层随手丢一张图进来怎么办」。`ProfileImage` 测量三个量——中间调占比、Otsu 阈值、彩色像素占比——`SuggestOptions` 据此给出建议；`DrawImage` **不会自己去测**，判错藏在绘制里没法排查。测量与决定分开，是为了让判错可见。
  ...
  - **红色分离度** 决定要不要关红色。红平面判定 `R>170 且 G<170` 收得太松，皮肤、粉色头发、木质全都命中——三张动画素材在不关红色时**大面积泛红**。判据是：在触发红色的像素里取 R−G 的中位数，实测真红 122–186（tiktok 186、article 135、朋友圈 127、线稿的嘴 122），假红 9–72（粉发 67–72、暖色调 43、暖灰 9），中间空 50 点，切在 100。对应 `ImageOptions.DisableRed`。
    早先用「彩色像素占比」做判据是**不够的**：它抓得住近乎单色的猫，抓不住动画——那些图确实有颜色，只是那颜色不是红。
  - **`EnhanceContrast`** 是连续调素材的前处理。深色主体需要强一些（猫在 2.4 下最清楚），赛璐璐动画需要弱一些（2.4 会过锐糊脸，1.2–1.4 合适）。**profile 里没有任何测量能区分这两种情况**——暗部占比在猫（12.8%）和动画广角（20.2%）上是反的，所以没有做成自动规则，example 里统一用 1.4，强度留给调用方。
    另外注意这个前处理**曾经掩盖过红色泛滥的 bug**：它输出灰度，顺带让红平面失效，所以只有在去掉锐化对照时才暴露出来。
  - 运行 `go run ./examples/gallery` 重新生成 `gallery.png` 联系表和 `out/` 下每张素材的 296×128 面板图；`-payload-for <name>` 可导出单张的真机 payload。素材来源与许可风险见 `examples/gallery/assets/SOURCE`。**尚未上真机确认。**
- `examples/card_showcase/card_showcase.png` 是把全部能力组合成一张实际界面的样例：头像用 `ClipPath` 裁成圆形（圆角来自裁剪而非事后覆盖），外圈用不足 360° 的圆弧当电量表盘（开放曲线，居中对齐），电量条的填充部分是裁进圆角里的斜纹图案，页脚是点阵图案中挖出白底文字，另有星形多边形、虚线分隔、中英混排多字号文字。整张图经 `DisplayList` 记录后重放。**这是三张图里唯一同时用满黑/白/红三色的**——红色不是画上去的，是源图里嘴部的红橙色命中了 4.2 节的红平面判定。
  **深色主体的照片是这块屏最难的一类素材，需要前处理。** 实测：一张黑猫照片直接 Floyd–Steinberg，72px / 96px / 124px 三种尺寸配 96 / 128 / 168 三档阈值，九种组合全部糊成一团黑，眼睛（暗底上的中灰调）始终不出现——问题不在阈值，主体本身内部对比就不够。先做非锐化掩蔽（局部对比增强，半径 10、强度 2.4）后眼睛、耳廓和胡须才浮出来。线稿则相反，不需要任何前处理，且必须用 `DitherThreshold`：Floyd–Steinberg 会把抗锯齿边缘的误差扩散到本该纯白的大片区域，实测糊成噪点，与 4.4 节记录一致。
  另外照片背景里的暖色（木质、米色）会命中 4.2 节的红平面判定，在本无红色的主体上撒出红点。**注意这里有个容易误判的交互**：对比增强按亮度计算并输出灰度图，R 等于 G 之后红色判定 `R>170 且 G<170` 已经不可能成立，红色其实是被灰度化顺带关掉的——实测把 `RedThreshold: 255` 去掉，输出零像素变化。该行现在只是显式声明意图，一旦调用方保留彩色就会真正生效。想主动关红色目前只能靠 `RedThreshold: 255` 这种「没有比 255 更亮」的取巧写法，`ImageOptions` 缺一个直白的开关。素材来源与许可风险见 `examples/card_showcase/SOURCE`。**尚未上真机确认。**

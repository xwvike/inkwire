...
func boundsCases() []struct {
	...
	return []struct {
		...
		{"StrokeEllipseDashed", func(d *DisplayList) { d.StrokeEllipse(Upright(image.Rect(10, 20, 80, 55)), dashed) }},
		{"StrokeRoundRectDashed", func(d *DisplayList) { d.StrokeRoundRect(image.Rect(8, 8, 70, 50), 12, dashed) }},
		{"DrawArcThick", func(d *DisplayList) { d.DrawArc(Upright(image.Rect(10, 10, 80, 70)), 200, 300, thick) }},
		{"DrawArcNegativeSweep", func(d *DisplayList) { d.DrawArc(Upright(image.Rect(10, 10, 80, 70)), 40, -190, thin) }},
		{"FillPie", func(d *DisplayList) { d.FillPie(Upright(image.Rect(10, 10, 80, 70)), -90, 125, InkRed) }},
		{"FillChord", func(d *DisplayList) { d.FillChord(Upright(image.Rect(10, 10, 80, 70)), 20, 210, InkBlack) }},
...
func TestDisplayListRecordsWhateverTheCanvasPaints(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{"ellipse, dash wider than the box",
			func(c *Canvas) { c.StrokeEllipse(Upright(rect), wide) },
			func(d *DisplayList) { d.StrokeEllipse(Upright(rect), wide) }},
		...
		{"full arc, dash wider than the box",
			func(c *Canvas) { c.DrawArc(Upright(rect), 0, 360, wide) },
			func(d *DisplayList) { d.DrawArc(Upright(rect), 0, 360, wide) }},
	} {
		t.Run(test.name, func(t *testing.T) {

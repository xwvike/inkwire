...
func (c *Canvas) StrokeRect(rect image.Rectangle, width int, ink Ink) {
	if width <= 0 || rect.Empty() {
		return
	}
	width = min(width, min(rect.Dx(), rect.Dy()))
	c.FillRect(image.Rect(rect.Min.X, rect.Min.Y, rect.Max.X, rect.Min.Y+width), ink)
	c.FillRect(image.Rect(rect.Min.X, rect.Max.Y-width, rect.Max.X, rect.Max.Y), ink)
	c.FillRect(image.Rect(rect.Min.X, rect.Min.Y+width, rect.Min.X+width, rect.Max.Y-width), ink)
	c.FillRect(image.Rect(rect.Max.X-width, rect.Min.Y+width, rect.Max.X, rect.Max.Y-width), ink)
}

// DrawLine uses integer Bresenham rasterization and includes both endpoints.
func (c *Canvas) DrawLine(from, to image.Point, ink Ink) {
	x0, y0 := from.X, from.Y
	x1, y1 := to.X, to.Y
	dx := abs(x1 - x0)
	sx := -1

...
func renderProbe(t *testing.T, body, css string) (*display.Frame, string) {
	t.Helper()
	page, err := Compile(`<div class="page">`+body+`</div>`,
	...
	// The probe writes its picture out because this package no longer opens
	// one: an img element compiles to the name of a file, and the file is
	// read by the decoder along with everything else about the document.
	document, err := (scene.Decoder{BaseDir: probeDirectory(t)}).Decode(bytes.NewReader(page.JSON))
	if err != nil {
		return nil, said + err.Error()
	}
...
// probeDirectory holds the one picture the probes point at, a checkerboard
// small enough that any treatment of it is visible.
func probeDirectory(t *testing.T) string {
	t.Helper()
	directory := t.TempDir()

...
func TestASceneElementDrawsWhatCSSCannotDescribe(t *testing.T) {
	...
	resolver := Compiler{Scenes: func(reference SceneRef) (compose.Node, error) {
		source := []byte(reference.Inline)
		if reference.Src != "" {
			read, err := os.ReadFile(dir + reference.Src)
			if err != nil {
				return nil, err
			}
			source = read
		}
		return (scene.Decoder{BaseDir: dir}).DecodeNode(source)
	}}
	...
	if bare := inkOf(t, plain.Root); bare >= drawn {
...
func TestAnInlineSceneIsDrawn(t *testing.T) {
	resolver := Compiler{Scenes: func(reference SceneRef) (compose.Node, error) {
		...
		return (scene.Decoder{}).DecodeNode([]byte(reference.Inline))
	}}
	document, err := resolver.Compile(
	...
	if drawn := inkOf(t, document.Root); drawn == 0 {
		t.Error("the inline scene drew nothing")
	}
}

func inkOf(t *testing.T, root compose.Node) int {
	t.Helper()
	composed, err := compose.NewDefaultCompiler()
	if err != nil {
		t.Fatal(err)
	}
	compiled, _, err := composed.Compile(compose.Document{Root: root})
	if err != nil {
		t.Fatal(err)
	}
